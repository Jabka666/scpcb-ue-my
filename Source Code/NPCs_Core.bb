					...
					Else
						Select n\State
							Case 0.0 ; ~ Wandering around
								...
								Else
									If n\PathStatus = 2 Then
										n\PathTimer = n\PathTimer - (fps\Factor[0] * 2.0)
										n\CurrSpeed = 0.0
										If Rand(1, 35) = 1 Then
											RotateEntity(n\Collider, 0.0, Rnd(360.0), 0.0, True)
										EndIf
							...
							Case 1.0
								...
								If MeNPCSeesPlayer(n) = 1 Then
									...
									DetectDistance = EntityDistanceSquared(n\Collider, me\Collider)
				...
				If PlayerRoom\RoomTemplate\Name = "room860" Then
					...
					Select n\State
						...
						Case 3.0 ; ~ Runs towards the player and attacks
							...
							If (Dist < 1.21 Lor (n\Frame > 451.0 And n\Frame < 493.0) Lor me\KillTimer < 0.0) Then
								msg\DeathMsg = ""
			...
			Case NPCType066
				...
				Select n\State
					...
					Case 1.0 ; ~ Staring at the player
						...
						If Dist < 1.21 + PowTwo(n\LastDist) Then n\State = Rand(2.0, 3.0)
						;[End Block]
				...
				If n\State > -1.0 Then
					If Dist < PowTwo(HideDistance) Then
						...
						If wi\NightVision = 0 Then
							HideEntity(n\OBJ)
							If (Not chs\Notarget) Then
								If Dist < 1.21 And n\Reload =< 0.0 And msg\Timer =< 0.0 Then
									Select Rand(6)
										Case 1
											;[Block]
