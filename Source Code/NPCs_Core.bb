...
Function TeleportCloser%(n.NPCs)
	...
	If ClosestWaypoint <> Null
		TeleportEntity(n\Collider, EntityX(ClosestWaypoint\OBJ, True), EntityY(ClosestWaypoint\OBJ, True) + 0.2, EntityZ(ClosestWaypoint\OBJ, True), n\CollRadiusH, True, 4.0)
...
Function NPCSeesPlayer%(n.NPCs, Dist#, Angle# = 75.0)
	...
	If n\NPCType <> NPCTypeMTF
		If Dist2 > (Dist * Dist) Lor n\BlinkTimer <= 0.0
			Return(0)
		Else
			...
			If ((me\SndVolume * me\SndVolume) > Dist2 Lor (DeltaYawVal < Angle)) And Visible
				Return(1)
			Else
				Return(0)
			EndIf
	...
	Else
		...
		If Dist2 < (Dist * Dist)
			If (me\SndVolume * me\SndVolume) > Dist2 Then ReturnState = 2
...
Function ChangeNPCTextureID%(n.NPCs, TextureID%, MatState% = -1)
	...
	EntityTexture(n\OBJ, Tex)
	If MatState <> -1 Then UpdateEntityMaterial(n\OBJ, MatState)
	If n\NPCType = NPCType173
		EntityTexture(n\OBJ2, Tex)
		If MatState <> -1 Then UpdateEntityMaterial(n\OBJ2, MatState)
	EndIf
	DeleteSingleTextureEntryFromCache(Tex) : Tex = 0
End Function
