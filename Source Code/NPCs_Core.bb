...
Function UpdateNPCs%()
	...
	For n.NPCs = Each NPCs
						...
						Select n\State
							...
							Case 9.0 ; ~ Attacks
								...
								If (Not me\Terminated)
									...
									If n\LastSeen = 0
										n\SoundCHN = PlaySound2(LoadTempSound("SFX\SCP\966\Echo" + Rand(3) + ".ogg"), Camera, n\Collider, 10.0, 1.0, True)
										n\LastSeen = 1
									EndIf
		...
		If n\IsDead And n\GravityMult = 1.0
			...
			n\GravityMult = 0.0
		EndIf
		If GravityDist < PowTwo(opt\CameraFogFar * LightVolume * 1.3) Lor n\NPCType = NPCType1499_1
...
Function SetNPCFrame%(n.NPCs, Frame#)
	...
	If EntityDistanceSquared(n\Collider, me\Collider) > PowTwo(opt\CameraFogFar * LightVolume * 1.3)
		If n\AnimTimer <= 0.0
