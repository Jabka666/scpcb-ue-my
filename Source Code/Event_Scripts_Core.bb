	...
	If (Not Skip)
		If e\EventState = 1.0
			...
			ElseIf e\EventState2 = 5.0
				...
				For r.Rooms = Each Rooms
					If r <> PlayerRoom
						If EntityDistanceSquared(r\OBJ, n_I\Curr049\Collider) < PowTwo(HideDistance * 2.0) And EntityDistanceSquared(r\OBJ, n_I\Curr049\Collider) > PowTwo(HideDistance)
				...
				If e\EventState < 4.0
					...
					Else
						If EntityDistanceSquared(me\Collider, n_I\Curr173\Collider) > 16.0 And (Not PlayerSees173(n_I\Curr173)) And PlayerRoom <> e\room
							...
							TeleportEntity(n_I\Curr173\Collider, x, y, z, n_I\Curr173\CollRadius, True)
							n_I\Curr173\CurrentRoom = e\room
	...
	If PlayerRoom = e\room
		...
		If EntityY(me\Collider) < (-4600.0) * RoomScale
			...
			If e\room\NPC[3] = Null
				...
				n.NPCs = CreateNPC(NPCTypeD, x2, y2, z2)
				n\State3 = -1.0 : n\IsDead = True
				ChangeNPCTextureID(n, NPC_CLASS_D_VICTIM_939_1_TEXTURE)
				SetNPCFrame(n, 40.0)
				RotateEntity(n\Collider, 0.0, e\room\Angle + 90.0, 0.0, True)
				...
				n.NPCs = CreateNPC(NPCTypeD, x2, y2, z2)
				n\State3 = -1.0 : n\IsDead = True
				ChangeNPCTextureID(n, NPC_CLASS_D_VICTIM_939_2_TEXTURE)
				SetNPCFrame(n, 19.0)
				RotateEntity(n\Collider, 0.0, e\room\Angle, 0.0, True)
				SetEmitter(e\room, x2, y2, z2, 30)
				For i = 0 To 2
					n.NPCs = CreateNPC(NPCTypeCockroach, x2 + Rnd(-0.2, 0.2), (e\room\y - 5632.0 * RoomScale) + 0.05, z2 + Rnd(-0.2, 0.2))
					RotateEntity(n\Collider, EntityPitch(n\Collider), Rnd(360.0), EntityRoll(n\Collider))
...
Function UpdateEvent_Room2_MT%(e.Events)
	If PlayerRoom = e\room
		...
		If e\room\mt = Null
			...
			Local it.Items, d.Doors, wayp.WayPoints
			Local Angle#, SinValue#, CosValue#, Dist#
				...
				For iX = 0 To MTGridSize - 1
					If e\room\mt\Grid[iX + (iY * MTGridSize)] > 0
						...
						If e\room\mt\Grid[iX + (iY * MTGridSize)] = MT_GENERATOR
							...
							p.Props = CreateProp(Null, "tank2.b3d", e\room\x + (iX * 2.0) - (SinValue * 977.0 * RoomScale) + (CosValue * 320.0 * RoomScale), e\room\y + MTGridY - (144.0 * RoomScale), e\room\z + (iY * 2.0) + (CosValue * 977.0 * RoomScale) + (SinValue * 320.0 * RoomScale), 0.0, Angle + 180.0, 0.0, Scale, Scale, Scale, True, 0, "")
							EntityParent(p\OBJ, TempInt)
						...
						ElseIf e\room\mt\Grid[iX + (iY * MTGridSize)] = MT_FIRST_ELEVATOR Lor e\room\mt\Grid[iX + (iY * MTGridSize)] = MT_SECOND_ELEVATOR
							...
							p.Props = CreateProp(Null, "lamp_e.b3d", e\room\x + (iX * 2.0) + (SinValue * 252.0 * RoomScale) + (CosValue * 560.0 * RoomScale), e\room\y + MTGridY + (413.6 * RoomScale), e\room\z + (iY * 2.0) + (CosValue * 252.0 * RoomScale) + (SinValue * 560.0 * RoomScale), 0.0, Angle + 270.0, 90.0, Scale, Scale, Scale, False, 0, "")
							EntityParent(p\OBJ, TempInt)
							p.Props = CreateProp(Null, "lamp_e.b3d", e\room\x + (iX * 2.0) - (SinValue * 252.0 * RoomScale) + (CosValue * 560.0 * RoomScale), e\room\y + MTGridY + (413.6 * RoomScale), e\room\z + (iY * 2.0) - (CosValue * 252.0 * RoomScale) + (SinValue * 560.0 * RoomScale), 0.0, Angle + 90.0, 90.0, Scale, Scale, Scale, False, 0, "")
							EntityParent(p\OBJ, TempInt)
		...
		If InFacility = UpperFloor
			...
			If e\EventState = 0.0
				...
				If (Not n_I\Curr106\Contained) And n_I\Curr106\State < 2.0 And (Not (chs\NoTarget Lor I_268\InvisibilityOn))
					n_I\Curr106\EnemyX = EntityX(e\room\Objects[Temp], True) : n_I\Curr106\EnemyY = EntityY(me\Collider) : n_I\Curr106\EnemyZ = EntityZ(e\room\Objects[Temp], True)
					n_I\Curr106\State = 2.0
				EndIf
	...
	If e\room\Dist < 14.0
		...
		Else
			...
			If e\EventState > 0.35
				e\EventState = Max(e\EventState - (fps\Factor[0] * 0.00015), 0.35)
...
Function UpdateEvent_Room2_7_HCZ_Guard%(e.Events)
	Select e\EventState
		Case 0.0
			;[Block]
	...
	If PlayerRoom = e\room
		If e\EventState = 1.0 And RemoteDoorOn
			If EntityDistanceSquared(me\Collider, e\room\RoomDoors[0]\OBJ) < 4.0
				...
				EndIf
			EndIf
		ElseIf e\EventState = 0.0 And e\room\RoomDoors[0]\Open
			RemoveEvent(e)
...
Function UpdateEvent_106_Sinkhole%(e.Events)
	...
	ElseIf PlayerRoom = e\room
		...
		If e\room\Dist < 0.5 And (Not chs\NoTarget)
			...
			MakeMeUnplayable()
			PositionEntity(me\Collider, CurveValue(EntityX(e\room\OBJ), EntityX(me\Collider), 10.0), CurveValue(EntityY(e\room\OBJ) - e\EventState2, EntityY(me\Collider), 25.0), CurveValue(EntityZ(e\room\OBJ), EntityZ(me\Collider), 10.0), True)
...
Function UpdateEvent_Brownout%(e.Events)
	...
	If Rand(100) = 1
		If IsRoomAdjacent(PlayerRoom, e\room) And InFacility = NullFloor
			If e\EventState2 < 0.5 Then me\LightBlink = Max(Rnd(1.0, 2.0), me\LightBlink)
			Temp = True
		EndIf
	EndIf
	If PlayerRoom = e\room
