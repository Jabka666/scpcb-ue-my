	...
	Else
		If e\room\Dist < 7.0
			...
			If I_005\ChanceToSpawn = 3
				TFormPoint(375.0, 65.0, -875.0, e\room\OBJ, 0)
			...
			Else
				TFormPoint(-296.0, 65.0, -240.0, e\room\OBJ, 0)
				n.NPCs = CreateNPC(NPCTypeGuard, TFormedX(), TFormedY(), TFormedZ())
				n\State = 8.0
				NPCIsDead(n, NPC_IS_DEAD_PRE)
				SetNPCFrame(n, 288.0)
	...
	If e\EventState = 0.0
		If PlayerRoom = e\room
			...
			If e\room\NPC[1] <> Null
				PositionEntity(e\room\NPC[1]\Collider, e\room\x, e\room\y + 0.35, e\room\z - 1.0 + 2048.0 * RoomScale, True)
				ResetEntity(e\room\NPC[1]\Collider)
			Else
				e\room\NPC[1] = CreateNPC(NPCTypeD, e\room\x, e\room\y + 0.35, e\room\z - 1.0 + 2048.0 * RoomScale)
				ChangeNPCTextureID(e\room\NPC[1], NPC_CLASS_D_FRANKLIN_TEXTURE)
			EndIf
			
			If e\room\NPC[7] <> Null
				PositionEntity(e\room\NPC[7]\Collider, e\room\x + 32.0 * RoomScale, e\room\y + 0.35, e\room\z + 3116.0 * RoomScale)
				ResetEntity(e\room\NPC[7]\Collider)
			Else
				e\room\NPC[7] = CreateNPC(NPCTypeD, e\room\x + 32.0 * RoomScale, e\room\y + 0.35, e\room\z + 3116.0 * RoomScale)
			...
			If e\room\NPC[2] <> Null
				PositionEntity(e\room\NPC[2]\Collider, e\room\x, e\room\y + 0.35, e\room\z + 2576.0 * RoomScale, True)
				ResetEntity(e\room\NPC[2]\Collider)
			Else
				e\room\NPC[2] = CreateNPC(NPCTypeGuard, e\room\x, e\room\y + 0.35, e\room\z + 2576.0 * RoomScale)
			EndIf
			...
			If e\EventState2 = 0.0
				If e\EventState > 900.0 And e\room\RoomDoors[3]\Open
					If e\EventState - fps\Factor[0] <= 900.0 
						...
						NPCIsDead(e\room\NPC[7], NPC_IS_DEAD)
...
Function UpdateEvent_Cont1_173_Intro%(e.Events)
	...
	If PlayerRoom = e\room
		...
		If e\EventState = 0.0
			...
			TFormPoint(-3424.0, -138.0, -2208.0, e\room\OBJ, 0)
					...
					ElseIf e\EventState3 < 20000.0
						...
						If e\EventState3 < 14100.0
							...
							ElseIf e\EventState3 < 14065.0
								...
								me\BigCameraShake = 3.0
								NPCIsDead(e\room\NPC[1], NPC_IS_DEAD)
	...
	If PlayerRoom = e\room
		...
		If e\EventState = 0.0 Lor e\EventStr <> "LoadDone"
			...
			If e\room\RoomDoors[1]\Open Lor e\room\RoomDoors[2]\Open
				...
				TFormPoint(-1055.0, -62.0, 650.0, e\room\OBJ, 0)
	...
	Else
		...
		Else
			...
			If e\EventState > 70.0 * 7.5 And e\EventState - fps\Factor[0] <= 70.0 * 7.5
				NPCIsDead(e\room\NPC[0], NPC_IS_DEAD)
...
Function UpdateEvent_Room2_Elevator%(e.Events)
	If e\EventState = 0.0
		If e\room\Dist < 8.0 And e\room\Dist > 0.0
			e\room\NPC[0] = CreateNPC(NPCTypeGuard, e\room\x, 0.35, e\room\z)
	...
	If PlayerRoom = e\room
		...
		If Temp
			...
			Select e\EventState
				...
				Case 25.0
					;[Block]
					e\room\NPC[0] = CreateNPC(NPCTypeD, e\room\x + Cos(e\room\Angle - 90.0) * 760.0 * RoomScale, e\room\y + 0.4, e\room\z + Sin(e\room\Angle - 90.0) * 760.0 * RoomScale)
					e\room\NPC[0]\State3 = -1.0
					NPCIsDead(e\room\NPC[0], NPC_IS_DEAD_PRE)
	...
	If PlayerRoom = e\room
		...
		Select e\EventState
			Case 1.0
				...
				TFormPoint(-139.0, 4968.0, 655.0, e\room\OBJ, 0)
				e\room\NPC[0] = CreateNPC(NPCTypeD, TFormedX(), TFormedY(), TFormedZ())
				e\room\NPC[0]\State = -1.0
...
Function UpdateEvent_Cont2C_066_1162_ARC%(e.Events)
	...
	If e\EventState4 = 0.0
		If e\room\Dist < 8.0
			If n_I\Curr066 = Null
				n_I\Curr066 = CreateNPC(NPCType066, EntityX(e\room\OBJ), e\room\y + 0.15, EntityZ(e\room\OBJ))
			Else
				If n_I\Curr066\State <> 66.0
					TeleportEntity(n_I\Curr066\Collider, EntityX(e\room\OBJ), e\room\y + 0.15, EntityZ(e\room\OBJ), n_I\Curr066\CollRadiusH, True)
...
Function UpdateEvent_Room3_Storage%(e.Events)
	If PlayerRoom = e\room
		...
		If PlayerY < (-4600.0) * RoomScale
			...
			If e\room\NPC[3] = Null
				TFormPoint(3372.0, -5565.0, 6294.0, e\room\OBJ, 0)
				
				Local y# = TFormedY()
				
				e\room\NPC[4] = CreateNPC(NPCTypeD, TFormedX(), y, TFormedZ())
				e\room\NPC[4]\State3 = -1.0
				NPCIsDead(e\room\NPC[4], NPC_IS_DEAD_PRE)
				...
				TFormPoint(1083.0, 0.0, 989.0, e\room\OBJ, 0)
				e\room\NPC[5] = CreateNPC(NPCTypeD, TFormedX(), y, TFormedZ())
				e\room\NPC[5]\State3 = -1.0
				NPCIsDead(e\room\NPC[5], NPC_IS_DEAD_PRE)
				...
				y = TFormedY()
				n.NPCs = CreateNPC(NPCTypeCockroach, TFormedX(), y + 0.05, TFormedZ())
				n\State = 2.0
				RotateEntity(n\Collider, EntityPitch(n\Collider), Rnd(360.0), EntityRoll(n\Collider))
				
				TFormPoint(3063.0, 0.0, 1354.0, e\room\OBJ, 0)
				n.NPCs = CreateNPC(NPCTypeCockroach, TFormedX(), y + 0.05, TFormedZ())
				n\State = 2.0
				RotateEntity(n\Collider, EntityPitch(n\Collider), Rnd(360.0), EntityRoll(n\Collider))
				
				TFormPoint(1188.0, 0.0, 1114.0, e\room\OBJ, 0)
				n.NPCs = CreateNPC(NPCTypeCockroach, TFormedX(), y + 0.05, TFormedZ())
				n\State = 2.0
				RotateEntity(n\Collider, EntityPitch(n\Collider), Rnd(360.0), EntityRoll(n\Collider))
			...
			Else
				...
				If (Not e\room\RoomDoors[0]\Open)
					For i = 0 To 1
						If UpdateLever(e\room\RoomLevers[i]\OBJ)
							...
							e\SoundCHN2 = PlaySoundEx(e\Sound2, Camera, e\room\RoomDoors[0]\OBJ, 400.0)
							Exit
						EndIf
					Next
				Else
				...
				UpdateSoundOrigin(e\SoundCHN2, Camera, e\room\RoomDoors[0]\OBJ, 400.0)
				...
				If UpdateButton(Null, e\room\Objects[3]) And mo\MouseHit1
					...
					mo\MouseHit1 = False
				EndIf
				RotateEntity(e\room\Objects[2], Sin(Float(MilliSec * 0.05)), 0.0, -Sin(Float(MilliSec * 0.03)))
				If AmbientSFX(3, 9) = 0
					AmbientSFX(3, 9) = LoadSound_Strict("SFX\Ambient\General\Ambient9.ogg")
				Else
					e\SoundCHN = LoopSoundEx(AmbientSFX(3, 9), e\SoundCHN, Camera, e\room\Objects[2], 6.0, 1.5)
				EndIf
				If PlayerY < -6400.0 * RoomScale
				...
				If e\SoundCHN <> 0
					If IsStreamPlaying_Strict(e\SoundCHN)
						If Rand(4) = 1
							...
							UpdateEntityMaterial(e\room\Objects[1], DEFERRED_FULLBRIGHT Or DEFERRED_TRANSPARENT, e\EventState3)
							If EntityHidden(e\room\Objects[1]) Then ShowEntity(e\room\Objects[1])
						ElseIf Rand(10) = 1
							If (Not EntityHidden(e\room\Objects[1])) Then HideEntity(e\room\Objects[1])
						EndIf
...
Function UpdateEvent_Cont1_106%(e.Events)
	...
	If PlayerRoom = e\room
			...
			If EntityY(me\Collider) < (-6400.0) * RoomScale
				...
				ElseIf e\EventState = 1.0 ; ~ Bone was broken
					...
					If e\EventState3 > 0.0 And e\EventState3 < 400.0
						AnimateEx(e\room\Objects[3], AnimTime(e\room\Objects[3]), 1.0, 120.0, 0.35, False)
		...
		Else
			...
			e\room\NPC[0] = CreateNPC(NPCTypeD, TFormedX(), TFormedY(), TFormedZ())
			e\room\NPC[0]\State3 = -1.0 : e\room\NPC[0]\HideFromNVG = True
			NPCIsDead(e\room\NPC[0], NPC_IS_DEAD_PRE)
			RotateEntity(e\room\NPC[0]\Collider, 0.0, e\room\Angle + 180.0, 0.0, True)
			...
			NPCIsDead(e\room\NPC[1], NPC_IS_DEAD_PRE)
...
Function UpdateEvent_Cont1_895%(e.Events)
	...
	If PlayerRoom = e\room
		...
		If e\room\NPC[0] = Null
			TFormPoint(321.0, -1469.0, 2416.0, e\room\OBJ, 0)
			e\room\NPC[0] = CreateNPC(NPCTypeD, TFormedX(), TFormedY(), TFormedZ())
			e\room\NPC[0]\State3 = -1.0
			NPCIsDead(e\room\NPC[0], NPC_IS_DEAD_PRE)
		...
		If e\room\NPC[1] <> Null
			If e\room\NPC[1]\IsDead = NPC_IS_NOT_DEAD Then UpdateSoundOrigin(e\room\NPC[1]\SoundCHN, Camera, e\room\NPC[1]\Collider, 100.0, 1.0, True)
		...
		TurnOffSecurityCam(e\room, (Not UpdateLever(e\room\RoomLevers[0]\OBJ, I_714\Using <> 2 And wi\GasMask <> 4 And wi\HazmatSuit <> 4 And EntityPitch(e\room\RoomLevers[0]\OBJ, True) < 0.0)))
	Else
	...
	Select e\EventState
		Case 0.0
			...
			If e\room\Dist < 7.0 And e\room\Dist > 0.0
				TFormPoint(685.0, 65.0, 980.0, e\room\OBJ, 0)
				e\room\NPC[0] = CreateNPC(NPCTypeGuard, TFormedX(), TFormedY(), TFormedZ())
				e\room\NPC[0]\State = 8.0
				NPCIsDead(e\room\NPC[0], NPC_IS_DEAD_PRE)
...
Function UpdateEvent_Room2_MT%(e.Events)
	...
	If PlayerRoom = e\room
		If EntityY(me\Collider, True) < -11000.0 * RoomScale
			...
			TurnEntity(e\room\Objects[0], FanSpeed, 0.0, 0.0)
			TurnEntity(e\room\Objects[1], FanSpeed, 0.0, 0.0)
			TurnEntity(e\room\Objects[2], FanSpeed, 0.0, 0.0)
			TurnEntity(e\room\Objects[3], FanSpeed, 0.0, 0.0)
			TurnEntity(e\room\Objects[4], FanSpeed / 4.0, 0.0, 0.0)
			TurnEntity(e\room\Objects[5], FanSpeed / 4.0, 0.0, 0.0)
				...
				Case 1.0
					...
					For i = 0 To 1
						Select i
							Case 0
								;[Block]
								TFormPoint(941.0, -12688.0, -553.0, e\room\OBJ, 0)
								;[End Block]
							Case 1
								;[Block]
								TFormPoint(3277.0, -12688.0, 2447.0, e\room\OBJ, 0)
								;[End Block]
						End Select
						CreateNPC(NPCType966, TFormedX(), TFormedY(), TFormedZ())
					Next
					
					TFormPoint(7993.0, -12688.0, 1637.0, e\room\OBJ, 0)
					n_I\Curr457 = CreateNPC(NPCType457, TFormedX(), TFormedY(), TFormedZ())
					
					TFormPoint(6806.0, -12658.0, -247.0, e\room\OBJ, 0)
					...
					TFormPoint(8029.0, -12686.0, 1416.0, e\room\OBJ, 0)
					n.NPCs = CreateNPC(NPCTypeD, TFormedX(), TFormedY(), TFormedZ())
					n\State3 = -1.0
					NPCIsDead(n, NPC_IS_DEAD_PRE)
					RotateEntity(n\Collider, 0.0, e\room\Angle + 180.0, 0.0, True)
...
Function UpdateEvent_Room2_Nuke%(e.Events)
	...
	If PlayerRoom = e\room
		...
		Else
			If e\EventState3 = 0.0
				If e\room\RoomDoors[2]\Open Then e\EventState3 = Rand(0.0, 1.0)
			Else
				e\EventState4 = Min(e\EventState4 + fps\Factor[0], 70.0 * 9.6)
				
				If e\EventState4 > 70.0 * 2.5 And e\EventState4 - fps\Factor[0] =< 70.0 * 2.5
					TFormPoint(889.0, 112.0, 379.0 - (758.0 * (e\EventState3 = 1.0)), e\room\OBJ, 0)
					e\room\NPC[1] = CreateNPC(NPCType1048_A, TFormedX(), TFormedY(), TFormedZ())
					e\room\NPC[1]\State = -1.0
					RotateEntity(e\room\NPC[1]\Collider, 0.0, e\room\Angle + 90.0 + (180.0 * (e\EventState3 = 1.0)), 0.0, True)
				...
				ElseIf e\EventState4 >= 70.0 * 9.6
					RotateEntity(e\room\Objects[e\EventState3], CurveAngle(-15.0, EntityPitch(e\room\Objects[e\EventState3], True), 15.0), e\room\Angle + (180.0 * (e\EventState3 = 1.0)), 0.0, True)
				EndIf
			EndIf
			
			Local SoundRange# = 2.0 + ((e\EventState3 = 0.0) * (e\EventState4 >= 70.0 * 9.6))
			Local SoundVol# = 0.8 + ((e\EventState3 = 0.0) * (e\EventState4 >= 70.0 * 9.6))
			
			e\SoundCHN = LoopSoundEx(snd_I\RoomAmbience[3], e\SoundCHN, Camera, e\room\Objects[0], SoundRange, SoundVol)
			e\SoundCHN2 = LoopSoundEx(snd_I\RoomAmbience[3], e\SoundCHN2, Camera, e\room\Objects[1], SoundRange, SoundVol)
		EndIf
	EndIf
End Function
	...
	If e\EventState = 0.0
		...
		If e\room\Dist > 0.0 And e\room\Dist < 5.0
			If IsRoomAdjacent(PlayerRoom, e\room)
				...
				TFormPoint(-352.0, 218.0, 0.0, e\room\OBJ, 0)
				...
				TFormPoint(-1328.0, 218.0, 528.0, e\room\OBJ, 0)
				e\room\NPC[0] = CreateNPC(NPCTypeGuard, TFormedX(), TFormedY(), TFormedZ())
	...
	If e\EventState = 0.0
		If PlayerRoom = e\room
			...
			Else
				Temp = -Sgn(TFormedZ())
			EndIf
			TFormPoint(-720.0, 19.0, 816.0 * Temp, e\room\OBJ, 0)
		...
		If EntityY(me\Collider) < (-8900.0) * RoomScale
			...
			Select e\EventState
				Case 0.0
					;[Block]
					If n_I\Curr173\Idle = 0
						TeleportEntity(n_I\Curr173\Collider, EntityX(e\room\Objects[3], True), EntityY(e\room\Objects[3], True), EntityZ(e\room\Objects[3], True), n_I\Curr173\CollRadiusH, True)
						n_I\Curr173\CurrentRoom = e\room
					EndIf
				...
				Case 1.0
					...
					If EntityDistanceSquared(me\Collider, e\room\Objects[0]) < 5.0
						If e\EventState2 = 0.0
							If EntityDistanceSquared(n_I\Curr173\Collider, e\room\Objects[3]) < 9.0
								If (Not PlayerSees173(n_I\Curr173)) And ((Not EntityInView(e\room\Objects[1], Camera)) Lor (me\BlinkTimer < -6.0 And me\BlinkTimer > -16.0))
									...
									EndIf
									e\SoundCHN2 = PlaySoundEx(LoadTempSound("SFX\Room\GlassBreak.ogg"), Camera, e\room\Objects[1]) 
									
									e\EventState2 = 1.0
								EndIf
							EndIf
						EndIf
						If I_008\Timer = 0.0 And me\Bloodloss > 0.0 Then InjurePlayer(0.0, 0.001)
						If wi\GasMask = 0 And wi\HazmatSuit = 0 Then me\EyeIrritation = Max(70.0, me\EyeIrritation)
					EndIf
					
					If InteractObject(e\room\Objects[0], 0.8, 1)
						DrawArrowIcon[2] = True
						RotateEntity(e\room\Objects[0], Clamp(EntityPitch(e\room\Objects[0], True) + Clamp(-mo\Mouse_Y_Speed_1, -10.0, 10.0), 5.0, 89.0), EntityYaw(e\room\Objects[0], True), 0.0, True)
					Else
						RotateEntity(e\room\Objects[0], EntityPitch(e\room\Objects[0], True) + Min(Cos(MilliSec) * 0.1, 89.0), EntityYaw(e\room\Objects[0], True), 0.0, True)
					EndIf
					...
					Else
						If e\room\RoomEmitters[0] = Null
							TFormPoint(-62.0, -9337.0, 889.0, e\room\OBJ, 0)
							e\room\RoomEmitters[0] = SetEmitter(e\room, TFormedX(), TFormedY(), TFormedZ(), 9)
				...
				Case 2.0
					...
					TFormPoint(-240.0, -9358.0, -635.0, e\room\OBJ, 0)
					...
					n\State = 3.0 : n\IdleTimer = 70.0 * -120.0
...
Function UpdateEvent_Cont2_049%(e.Events)
	If PlayerRoom = e\room
		...
		If EntityY(me\Collider) > (-2848.0) * RoomScale
			If Rand(2500) = 1
				TFormPoint(-1744.0, -320.0, -710.0, e\room\OBJ, 0)
				PlaySoundEx(LoadTempSound("SFX\SCP\Joke\B" + Rand(0, 3) + ".ogg"), Camera, GetDummyPivot(TFormedX(), TFormedY(), TFormedZ()), 5.0, 0.1)
			EndIf
		...
		Else
			...
			Select e\EventState
				Case 0.0
					;[Block]
					TFormPoint(592.0, -3438.0, -1424.0, e\room\OBJ, 0)
					...
					TFormPoint(64.0, -3438.0, -1000.0, e\room\OBJ, 0)
					n.NPCs = CreateNPC(NPCType049_2, TFormedX(), TFormedY(), TFormedZ())
					PointEntity(n\Collider, e\room\OBJ)
					TurnEntity(n\Collider, 0.0, e\room\Angle + 60.0, 0.0)
					
					e\SoundCHN = PlaySound_Strict(LoadTempSound("SFX\Room\Blackout.ogg"))
					If EntityDistanceSquared(e\room\RoomElevators[0]\FloorPoint, me\Collider) < EntityDistanceSquared(e\room\RoomElevators[1]\FloorPoint, me\Collider)
						...
						If z2
							...
							If e\EventState4 = 0.0
								...
								If EntityDistanceSquared(me\Collider, e\room\RoomElevators[0]\door2\FrameOBJ) < 9.0
									i = 0
								ElseIf EntityDistanceSquared(me\Collider, e\room\RoomElevators[1]\door2\FrameOBJ) < 9.0
									i = 1
								EndIf
								If i > -1
									If n_I\Curr049 <> Null
										If n_I\Curr049\State <> 66.0
											PositionEntity(n_I\Curr049\Collider, EntityX(e\room\RoomElevators[i]\FloorPoint, True), EntityY(e\room\RoomElevators[i]\FloorPoint, True), EntityZ(e\room\RoomElevators[i]\FloorPoint, True))
											n_I\Curr049\CurrentRoom = e\room
										EndIf
									Else
										n_I\Curr049 = CreateNPC(NPCType049, EntityX(e\room\RoomElevators[i]\FloorPoint, True), EntityY(e\room\RoomElevators[i]\FloorPoint, True), EntityZ(e\room\RoomElevators[i]\FloorPoint, True))
									EndIf
									PointEntity(n_I\Curr049\Collider, e\room\RoomElevators[i]\FloorPoint)
									ResetEntity(n_I\Curr049\Collider)
									GiveAchievement("049")
					...
					If e\EventState3 > (-70.0) * 4.0
						If me\FallTimer < -230.0
							...
							If e\EventState3 <= (-70.0) * 4.0
								...
								TFormPoint(64.0, -3375.0, 1000.0, e\room\OBJ, 0)
	...
	If PlayerRoom = e\room
		If EntityY(me\Collider) < (-1774.0) * RoomScale
			...
			If e\EventState = 0.0
				; ~ Spawn some stuff
				;[Block]
				TFormPoint(-2251.8, -2442.0, 3513.0, e\room\OBJ, 0)
					...
					If e\EventState4 = 0.0
						...
						Else
							...
							If e\EventState3 > 70.0 * 11.0
								If I_409\Timer > 0.0 Lor I_009\Timer > 0.0
									If e\room\RoomEmitters[0] = Null
...
Function UpdateEvent_Room3_2_HCZ_Guard%(e.Events)
	If e\room\Dist < 8.0
		...
		TFormPoint(-190.0, 68.0, 190.0, e\room\OBJ, 0)
		n.NPCs = CreateNPC(NPCTypeGuard, TFormedX(), TFormedY(), TFormedZ())
		n\State = 8.0
		NPCIsDead(n, NPC_IS_DEAD_PRE)
		SetNPCFrame(n, 288.0)
...
Function UpdateEvent_Cont3_009%(e.Events)
	...
	If PlayerRoom = e\room
		If e\room\NPC[0] = Null
			TFormPoint(382.0, -450.2, -263.0, e\room\OBJ, 0)
			e\room\NPC[0] = CreateNPC(NPCTypeD, TFormedX(), TFormedY(), TFormedZ())
			e\room\NPC[0]\State3 = -1.0 : e\room\NPC[0]\State = 66.0 : e\room\NPC[0]\IceTimer = 70.0 * 30.0
			NPCIsDead(e\room\NPC[0], NPC_IS_DEAD_PRE)
		...
		ElseIf e\EventState = 66.0
			...
			For n.NPCs = Each NPCs
				If n\IceTimer = 0.0 And n\CurrentRoom = e\room
					Select n\NPCType
						Case NPCType513_1, NPCType372, NPCType106, NPCType173, NPCTypeD, NPCTypeGuard, NPCTypeMTF
							;[Block]
							; ~ Skip
							;[End Block]
						Case NPCType999
		...
		Select e\EventState
			Case 0.0
				...
				If e\room\RoomDoors[0]\Open Lor e\room\RoomDoors[1]\Open
					...
					TFormPoint(0.0, 65.0, -297.0, e\room\OBJ, 0)
					e\room\NPC[0] = CreateNPC(NPCTypeD, TFormedX(), TFormedY(), TFormedZ())
					e\room\NPC[0]\State3 = -1.0
					NPCIsDead(e\room\NPC[0], NPC_IS_DEAD_PRE)
...
Function UpdateEvent_Room4_2_HCZ_D%(e.Events)
	If e\room\Dist < 8.0
		...
		TFormPoint(256.0, 68.0, 256.0, e\room\OBJ, 0)
		n.NPCs = CreateNPC(NPCTypeD, TFormedX(), TFormedY(), TFormedZ())
		n\State3 = -1.0
		NPCIsDead(n, NPC_IS_DEAD_PRE)
...
Function UpdateEvent_Room1_EZ_Guard%(e.Events)
	If e\EventState = 0.0
		If PlayerRoom = e\room
			TFormPoint(-944.0, 440.0, 20.0, e\room\OBJ, 0)
			e\room\NPC[0] = CreateNPC(NPCTypeGuard, TFormedX(), TFormedY(), TFormedZ())
			RotateEntity(e\room\NPC[0]\Collider, 0.0, e\room\Angle, 0.0, True)
...
Function UpdateEvent_Gate_A%(e.Events)
	If PlayerRoom = e\room
		...
		If e\EventState = 0.0
			...
			TFormPoint(1784.0, 1924.0, 4512.0, e\room\OBJ, 0)
			TargetY = TFormedY()
			e\room\NPC[0] = CreateNPC(NPCTypeGuard, TFormedX(), TargetY, TFormedZ())
			e\room\NPC[0]\State = 0.0 : e\room\NPC[0]\GravityMult = 0.0
			
			TFormPoint(-5048.0, 0.0, 4656.0, e\room\OBJ, 0)
			e\room\NPC[1]  = CreateNPC(NPCTypeGuard, TFormedX(), TargetY, TFormedZ())
			e\room\NPC[1]\State = 0.0 : e\room\NPC[1]\GravityMult = 0.0
			
			TFormPoint(-5615.0, 0.0, 6904.0, e\room\OBJ, 0)
			n.NPCs = CreateNPC(NPCTypeD, TFormedX(), TargetY, TFormedZ())
			...
			TFormPoint(1824.0, 218.0, 7056.0, e\room\OBJ, 0)
...
Function UpdateEvent_Gate_B%(e.Events)
	If PlayerRoom = e\room
		...
		If e\EventState = 0.0
			...
			TFormPoint(5203.0, 1412.0, -1739.0, e\room\OBJ, 0)
			...
			TFormPoint(6234.0, 34.0, -8451.0, e\room\OBJ, 0)
				...
				Else
					If me\SelectedEnding = -1
						...
						Else
							...
							TFormPoint(3600.0, -838.0, 6623.0, e\room\OBJ, 0)
							n.NPCs = CreateNPC(NPCTypeMTF, TFormedX(), TFormedY(), TFormedZ())
							e\room\NPC[4] = n
							TFormPoint(4352.0, 192.0, 255.0, e\room\OBJ, 0)
							n.NPCs = CreateNPC(NPCTypeMTF, TFormedX(), TFormedY(), TFormedZ())
							e\room\NPC[5] = n
...
Function UpdateEvent_Room2_EZ_035%(e.Events)
	...
	If Is035Released
		If e\room\Dist < 8.0
			If e\EventState = 0.0
				n.NPCs = CreateNPC(NPCTypeD, e\room\x, e\room\y + 64.0 * RoomScale, e\room\z)
				n\State3 = -1.0
				NPCIsDead(n, NPC_IS_DEAD_PRE)
				SetNPCFrame(n, 19.0)
...
Function UpdateEvent_Room2_IC%(e.Events)
	If PlayerRoom = e\room
		...
		TFormPoint(-1200.0, 65.0, 0.0, e\room\OBJ, 0)
		
		Local x1# = TFormedX(), y1# = TFormedY(), z1# = TFormedZ()
		
		CreateDecal(DECAL_BLOOD_2, x1, e\room\y + 0.005, z1, 90.0, Rnd(360.0), 0.0)
...
Function UpdateEvent_Room2_Medibay%(e.Events)
	If PlayerRoom = e\room
		If e\EventState = 0.0
			TFormPoint(-820.0, 218.0, -464.0, e\room\OBJ, 0)
...
Function UpdateEvent_Room2_Office%(e.Events)
	If e\room\Dist < 6.0
		TFormPoint(820.0, -326.0, 0.0, e\room\OBJ, 0)
	...
	If PlayerRoom = e\room
		...
		Select e\EventState2
			...
			Case PD_FourWayRoom
				...
				Pvt = GetDummyPivot(EntityX(e\room\Objects[8], True) - 1536.0 * RoomScale, e\room\y + 500.0 * RoomScale, EntityZ(e\room\Objects[8], True) + 608.0 * RoomScale)
...
Function UpdateEvent_Dimension_1499%(e.Events)
	...
	If PlayerRoom = e\room
		...
		If e\EventState < 2.0
			...
			If e\EventState3 < 70.0 * 30.0
				; ~ The King
				e\room\NPC.NPCs[0] = CreateNPC(NPCType1499_1, e\room\x - 1917.0 * RoomScale, e\room\y + 1866.0 * RoomScale, e\room\z + 2308.0 * RoomScale)
				...
				; ~ Guard next to king
				e\room\NPC.NPCs[1] = CreateNPC(NPCType1499_1, e\room\x - 1917.0 * RoomScale, e\room\y + 1866.0 * RoomScale, e\room\z + 2052.0 * RoomScale)
				...
				e\room\NPC.NPCs[2] = CreateNPC(NPCType1499_1, e\room\x + 4055.0 * RoomScale, e\room\y + 90.0 * RoomScale, e\room\z + 1884.0 * RoomScale)
				e\room\NPC[2]\PrevState = 3 : e\room\NPC[2]\Angle = 270.0
				RotateEntity(e\room\NPC[2]\Collider, 0.0, e\room\NPC[2]\Angle, 0.0)
				e\room\NPC.NPCs[3] = CreateNPC(NPCType1499_1, e\room\x + 4055.0 * RoomScale, e\room\y + 90.0 * RoomScale, e\room\z + 2876.0 * RoomScale)
				...
				; ~ More guards
				e\room\NPC.NPCs[4] = CreateNPC(NPCType1499_1, e\room\x - 1877.0 * RoomScale, e\room\y + 154.0 * RoomScale, e\room\z + 1071.0 * RoomScale)
				e\room\NPC[4]\PrevState = 3 : e\room\NPC[4]\Angle = 270.0
				RotateEntity(e\room\NPC[4]\Collider, 0.0, e\room\NPC[4]\Angle, 0.0)
				e\room\NPC.NPCs[5] = CreateNPC(NPCType1499_1, e\room\x - 1877.0 * RoomScale, e\room\y + 154.0 * RoomScale, e\room\z + 3503.0 * RoomScale)
				...
				; ~ Guard at stairs
				n.NPCs = CreateNPC(NPCType1499_1, e\room\x - 2761.0 * RoomScale, e\room\y + 90.0 * RoomScale, e\room\z + 3204.0 * RoomScale)
...
Function UpdateEvent_096_Spawn%(e.Events)
	...
	If e\room\Dist < 17.0
		...
		If e\EventState = 0.0
			...
			TFormPoint(x, 218.0, z, e\room\OBJ, 0)
			If n_I\Curr096 <> Null
	...
	If (Not n_I\Curr106\Contained)
		...
		If e\EventState > 0.0
			...
			If e\EventState > 200.0
				If e\room\NPC[0] = Null
					e\room\NPC[0] = CreateNPC(NPCTypeD, e\room\x, e\room\y + 862.0 * RoomScale, e\room\z)
					e\room\NPC[0]\State = -1.0 : e\room\NPC[0]\State3 = -1.0
...
Function UpdateEvent_Checkpoint%(e.Events)
	...
	If PlayerRoom = e\room
		...
		If e\EventState3 = 0.0
			If Rand(2) = 1
				TFormPoint(877.0, 132.0, 333.0, e\room\OBJ, 0)
				e\room\NPC[0] = CreateNPC(NPCType1048, TFormedX(), TFormedY(), TFormedZ())
				e\room\NPC[0]\State = 2.0
			EndIf
			e\EventState3 = 1.0
...
Function UpdateEvent_Tesla%(e.Events)
	...
	If e\room\Dist < 16.0
		...
		If Temp
			e\room\RoomTemplate\DisableDecals = 2
			If PrevLever <> Temp
				If Temp
					PlaySound_Strict(snd_I\TeslaPowerUpSFX)
					e\EventState = 0.0
		...
		If PlayerRoom = e\room
			...
			If e\room\NPC[0] = Null And Temp
				...
				EndIf
				e\room\NPC[0] = CreateNPC(NPCTypeClerk, EntityX(e\room\OBJ, True) + x1 * RoomScale, e\room\y + 0.35, EntityZ(e\room\OBJ, True) + z1 * RoomScale)
			...
			Case 0.0 ; ~ Idle state
				...
				If IsEqual(EntityX(me\Collider, True), x2, 1.0) And IsEqual(EntityZ(me\Collider, True), z2, 1.0) And IsEqual(EntityY(me\Collider, True), y2, 1.3)
					If (Not me\Terminated) And (Not chs\NoTarget)
						If e\room\NPC[0] = Null Lor e\room\NPC[0]\IsDead > NPC_IS_NOT_DEAD
							If ChannelPlaying(e\SoundCHN) Then StopChannel(e\SoundCHN) : e\SoundCHN = 0
				...
				EndIf
				For n.NPCs = Each NPCs
					If n\NPCType <> NPCType513_1 And n\IsDead = NPC_IS_NOT_DEAD
