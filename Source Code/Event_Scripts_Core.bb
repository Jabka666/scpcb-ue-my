...
Function UpdateEvent_Room2_SL%(e.Events)
	...
	If e\EventState = 1.0
		If e\EventState2 < 0.0
			If (Not e\EventState3) Then e\EventState2 = Min(e\EventState2 + fps\Factor[0], 0.0)
	...
	If PlayerRoom = e\room
		...
		If EntityY(me\Collider) < (-4600.0) * RoomScale
			...
			If wi\GasMask = 0 And wi\HazmatSuit = 0
				me\Suffocation = Min(me\Suffocation + (fps\Factor[0] * 1.05), 1500.0)
				If me\Suffocation >= 500.0 Then UpdateCough(1000)
				me\BlurTimer = Max(me\Suffocation / 2.0, me\BlurTimer)
				If me\Suffocation >= 1500.0 And me\FallTimer = 0.0 And (Not me\Terminated)
		...
		Else
			...
			ElseIf e\EventState < 200.0
				...
				If EntityDistanceSquared(n_I\Curr106\Collider, me\Collider) < 16.0
					Local Pvt% = GetDummyPivot(EntityX(n_I\Curr106\Collider), EntityY(n_I\Curr106\Collider), EntityZ(n_I\Curr106\Collider))
				...
				Else
					...
					Else
						If EntityDistanceSquared(me\Collider, n_I\Curr173\Collider) > 16.0 And (Not PlayerSees173(n_I\Curr173)) And PlayerRoom <> e\room
							...
							TeleportEntity(n_I\Curr173\Collider, TFormedX(), TFormedY(), TFormedZ(), n_I\Curr173\CollRadius, True)
							n_I\Curr173\CurrentRoom = e\room
							e\EventState = 11.0
						EndIf
					EndIf
		...
		Else
			...
			ElseIf e\room\NPC[0] <> Null
				...
				Else
					...
					Local Pvt% = GetDummyPivot(EntityX(e\room\NPC[1]\Collider), EntityY(e\room\NPC[1]\Collider) + 0.2, EntityZ(e\room\NPC[1]\Collider))
