			...
			If d\Open Then
				...
				Else
					...
					If d\TimerState > 0.0 Then
						...
						If d\TimerState = 0.0 Then d\Open = (Not d\Open) : d\SoundCHN = PlaySound2(CloseDoorSFX(d\Dir, Sound%), Camera, d\OBJ)
					EndIf
							...
							If d\OpenState < 15.0 And d\OpenState + FPSfactor >= 15.0
								If ParticleAmount = 2
									For i = 0 To Rand(75, 99)
										...
										RotateEntity(Pvt, 0, Rnd(360.0), 0.0)
...
Function MilliSecs2()
	...
	If RetVal < 0.0 Then RetVal = RetVal + 2147483648.0
	Return(RetVal)
End Function
...
Repeat
	...
	Else
		...
		If InfiniteStamina% Then Stamina = Min(100.0, Stamina + (100.0 - Stamina) * 0.01 * FPSfactor)
...
Function DrawGUI()
	...
	If HUDenabled Then 
		...
		If PlayerRoom\RoomTemplate\Name = "pocketdimension" Or Wearing714 > 0 Or Injuries >= 1.5 Or StaminaEffect > 1.0 Or WearingHazmat > 0 Or WearingVest = 2
			...
			Rect(x - 50 - 3, y - 3, 30 + 6, 30 + 6)
		Else
		    If InfiniteStamina = True Or StaminaEffect < 1.0 Or WearingGasMask = 2 Or Wearing1499 = 2
                Color(0, 200, 0)
			    Rect(x - 50 - 3, y - 3, 30 + 6, 30 + 6)
            EndIf 
		End If
...
Function InitNewGame()
	...
	For r.Rooms = Each Rooms
		...
		If (Not r\RoomTemplate\DisableDecals) Then
			If Rand(4) = 1 Then
				de.Decals = CreateDecal(Rand(2, 3), EntityX(r\OBJ) + Rnd(- 2, 2), 0.003, EntityZ(r\OBJ) + Rnd(-2, 2), 90.0, Rand(360.0), 0.0)
				de\Size = Rnd(0.1, 0.4) : ScaleSprite(de\OBJ, de\Size, de\Size)
				EntityAlpha(de\OBJ, Rnd(0.85, 0.95))
			EndIf
			
			If Rand(4) = 1 Then
				de.Decals = CreateDecal(0, EntityX(r\OBJ) + Rnd(- 2, 2), 0.003, EntityZ(r\OBJ) + Rnd(-2, 2), 90.0, Rand(360.0), 0.0)
	...
	Select item\itemtemplate\name
		...
		Case "Severed Hand", "Black Severed Hand"
			...
			Select Setting
				Case "rough", "coarse"
					;[Block]
					d.Decals = CreateDecal(3, x, 8 * RoomScale + 0.005, z, 90.0, Rand(360.0), 0.0)
					d\Size = 0.12 : ScaleSprite(d\OBJ, d\Size, d\Size)
					;[End Block]
				Case "1:1", "fine", "very fine"
					;[Block]
		...
		Case "SCP-513"
			...
			Select Setting
				Case "rough", "coarse"
					...
					Next
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.010, z, 90.0, Rand(360.0), 0.0)
