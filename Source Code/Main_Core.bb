...
Function Kill%(IsBloody% = False, Animated% = True)
	...
	If (Not me\Terminated)
		...
		If IsBloody
			Local Tex% = LoadTexture_Strict("GFX\Overlays\blood_overlay.png")
			...
			EntityBlend(t\OverlayID[OVERLAY_BLOODY], 2)
			...
			If me\Playable = 2 And me\FallTimer >= 0.0 And (Not me\Terminated)
				If (KeyDown(key\MOVEMENT_DOWN) Xor KeyDown(key\MOVEMENT_UP)) Lor (KeyDown(key\MOVEMENT_RIGHT) Xor KeyDown(key\MOVEMENT_LEFT)) Lor me\ForceMove > 0.0 
					If (Not me\Crouch) And (Not me\Zombie) And (KeyDown(key\SPRINT) And (Not InvOpen) And OtherOpen = Null) And me\Stamina > 0.0
						me\Stamina = me\Stamina - (fps\Factor[0] * (0.28 + (0.28 * (I_966\HasInsomnia + ((me\EyeIrritation > 70.0) * 0.5)))) * me\StaminaEffect)
								...
								If (Inventory(MouseSlot)\State > 0.0 And Inventory(MouseSlot)\ItemTemplate\ID = it_e_reader) Lor Inventory(MouseSlot)\ItemTemplate\ID = it_e_reader20 Lor Inventory(MouseSlot)\ItemTemplate\ID = it_e_readerulti
									Select SelectedItem\ItemTemplate\ID
										Case it_paper, it_oldpaper
											...
											If SelectedItem\ItemTemplate\Name = "Leaflet" Lor SelectedItem\ItemTemplate\Name = "Drawing" Lor SelectedItem\ItemTemplate\Name = "Note from Maynard" Lor SelectedItem\ItemTemplate\Name = "SCP-085" Lor SelectedItem\ItemTemplate\ID = it_oldpaper Lor SelectedItem\ItemTemplate\Name = "Newspaper"
...
Function UpdateVomit%()
	...
	If me\VomitTimer > 0.0
		...
		If (MilliSec Mod 4000) < 200 Then UpdateCough(50)
