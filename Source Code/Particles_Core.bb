...
Function UpdateDust%()
	...
	DustParticleChance = Max(60 - (25 * (opt\ParticleAmount > 0)) - (me\BigCameraShake > 0.0) * 35, 1)
...
Function UpdateBreathSteam%()
	If wi\HazmatSuit > 0 Lor wi\GasMask > 0 Lor opt\ParticleAmount = 0
		BreathSteamTimer = 0.0
		Return
	EndIf
	
	BreathSteamTimer = BreathSteamTimer + fps\Factor[0]
	If BreathSteamTimer > 70.0 * (Rnd(2.8, 3.2) - (ChannelPlaying(BreathCHN) * 1.5))
