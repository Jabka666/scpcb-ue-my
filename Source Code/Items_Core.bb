	...
	For i.Items = Each Items
		...
		If i\Nearby And (Not i\Picked)
			...
			If i\Dist < 1.44 And (ClosestItem = Null Lor i\Dist < EntityDistanceSquared(Camera, ClosestItem\Collider))
				If EntityInView(i\OBJ, Camera) And EntityVisible(i\Collider, Camera)
					If opt\DirectSight
	...
	Select item\ItemTemplate\ID
		...
		Case it_key0, it_key1, it_key2, it_key3, it_key4, it_key5, it_key6
			...
			Select Setting
				...
				Case SETTING_VERY_FINE
					;[Block]
					CurrAchvAmount = Max(((S2IMapSize(AchievementsIndex) - 4) - (S2IMapSize(UnlockedAchievements) - 1) - S2IMapContains(UnlockedAchievements, "apollyon")) * (5 + SelectedDifficulty\OtherFactors), 0)
					If Rand(0, CurrAchvAmount) = 0
						CreateItem("Key Card Omni", it_keyomni, x, y, z)
			...
			Select Setting
				...
				Case SETTING_VERY_FINE
					...
					If Rand(Max((RoomsAmount - (RoomsFound * 2)) * (1 + SelectedDifficulty\OtherFactors), 1)) = 1
						CreateItem("S-NAV Ultimate", it_navulti, x, y, z)
					Else
						CreateItem("S-NAV 3000", it_nav3000, x, y, z)
					EndIf
					;[End Block]
