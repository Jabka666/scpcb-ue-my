Function UpdateNPCType008_1_Surgeon%(n.NPCs)
	; ~ n\State: Main State
			...
			Case 3.0 ; ~ Player/NPC isn't visible, tries to find
				...
				If n\TargetUpdateTimer =< 0.0
					...
					For n2.NPCs = Each NPCs
						If (n2\NPCType = NPCTypeMTF Lor n2\NPCType = NPCType035_Tentacle) And n2\IsDead = NPC_IS_DEAD
...
Function UpdateNPCType008_1%(n.NPCs)
	...
	If n\IsDead = NPC_IS_NOT_DEAD
		...
		Select n\State
			...
			Case 1.0 ; ~ Stands up
				;[Block]
				AnimateNPC(n, 33.0, 63.0, 0.1, False)
				n\Angle = CurveAngle(EntityYaw(n\Collider, True), n\Angle, 20.0)
				If n\Frame > 62.9 Then n\State = 3.0
				;[End Block]
			Case 2.0 ; ~ Player/NPC is visible, tries to kill
				;[Block]
				n\State2 = Max(n\State2 - fps\Factor[0], 0.0)
			...
			Case 3.0 ; ~ Player/NPC isn't visible, tries to find
				...
				If n\CurrSpeed < 0.005
					If n\Frame < 131.0
						AnimateNPC(n, 112.0, 130.0, 0.3, False)
					Else
						AnimateNPC(n, 593.0, 669.0, 0.2)
					EndIf
				Else
				...
				If n\TargetUpdateTimer =< 0.0
					...
					For n2.NPCs = Each NPCs
						If (n2\NPCType = NPCTypeMTF Lor n2\NPCType = NPCType035_Tentacle) And n2\IsDead = NPC_IS_NOT_DEAD
							If NPCSeesNPC(n2, n) = 1
								n\Target = n2
								n\State2 = 70.0 * 2.0 ; ~ Give up after 2 seconds
								n\State = 2.0
			...
			Case 4.0 ; ~ Attacks
				...
				If n\Frame < 174.0
					AnimateNPC(n, 131.0, 174.0, 0.6, False)
					Attack = (n\Frame >= 155.0 And PrevFrame < 155.0)
					If n\Frame > 173.9
						SetNPCFrame(n, 62.0)
						n\State = 2.0 + me\Terminated
						Return
					EndIf
				Else
					AnimateNPC(n, 174.0, 200.0, 0.3, False)
					Attack = (n\Frame >= 189.0 And PrevFrame < 189.0)
					If n\Frame > 199.9
						SetNPCFrame(n, 62.0)
						n\State = 2.0 + me\Terminated
						Return
				...
				If n\Target <> Null
					If n\Target\IsDead > NPC_IS_NOT_DEAD
						n\Target = Null
						n\State = 3.0
					EndIf
				EndIf
						...
						For n2.NPCs = Each NPCs
							Select n2\NPCType
								Case NPCType008_1, NPCType008_1_Surgeon, NPCType049_2, NPCTypeMTF, NPCType1048_A
									;[Block]
									If EntityDistanceSquared(n\Collider, n2\Collider) < 3.24 And n2\IsDead = NPC_IS_NOT_DEAD
										n\Target = n2
										n\State = 2.0
										Return
									EndIf
	...
	Else
		...
		Select n\State
						...
						Else ; ~ Stands still and tries to find a path
							...
							If n\PathTimer > 70.0 * (4.0 - (2.0 * SelectedDifficulty\AggressiveNPCs))
								...
								While Int(n\State3) < 3.0
									...
									If n\PathStatus = PATH_STATUS_FOUND
										If n\Path[1] <> Null
											If n\Path[2] = Null And EntityDistanceSquared(n\Path[1]\OBJ, n\Collider) < 0.16 Then ErasePath(n)
										EndIf
										If n\Path[0] <> Null And n\Path[1] = Null Then ErasePath(n)
									EndIf
			...
			Case 4.0 ; ~ Going to surveillance room
				...
				Else
					If n\State3 = 6.0
						If EntityDistanceSquared(n\Collider, me\Collider) > 289.0
							ErasePath(n)
							n\PathTimer = 0.0
							n\PrevState = 0
							n\State3 = 0.0
						...
						If (Not (chs\NoTarget Lor I_268\InvisibilityOn))
							Select Rand(6)
								Case 1
									...
									For n2.NPCs = Each NPCs
										If EntityDistanceSquared(n\Collider, n2\Collider) < 64.0 And n2\IsDead = NPC_IS_NOT_DEAD
											Select n2\NPCType
		...
		Select n\State
			...
			Case 1.0 ; ~ Idling outside the map
				...
				If PlayerRoom\RoomTemplate\DisableDecals < 4
					Local TimerCountDown# = fps\Factor[0] * ((1.0 + n\State3) * (1.0 + SelectedDifficulty\AggressiveNPCs))
					...
					Select PlayerRoom\RoomTemplate\DisableDecals
						...
						Case 2
							;[Block]
							If n\State2 > 1400.0 Then n\State2 = n\State2 - TimerCountDown
							;[End Block]
						Case 3
							;[Block]
			...
			Case 4.0 ; ~ Hit by Tesla or HID
				...
				EndIf
				;[End Block]
			Case 5.0 ; ~ Sitting on the throne
				;[Block]
				If n\Frame < 668.0
				...
				Else
					If n\LastSeen = 0.0
						AnimateNPC(n, 668.0, 689.0, 0.2, False)
						If n\Frame > 688.9 Then n\LastSeen = 1.0
					Else
						AnimateNPC(n, 693.0, 668.0, -0.2, False)
						If n\Frame < 668.1
							SetNPCFrame(n, 605.0)
							n\LastSeen = 0.0
						EndIf
		...
		If n\Idle < 2
			...
			If n\Idle = 0
				...
				Else
					...
					If Dist > 184.96 ; ~ Move randomly from waypoint to another
						...
						If EntityDistanceSquared(n\Collider, n2\Collider) < 1.0
							Select n2\NPCType
								Case NPCType008_1, NPCType008_1_Surgeon, NPCType049_2, NPCTypeMTF, NPCTypeCockroach
									;[Block]
									If n2\IsDead = NPC_IS_NOT_DEAD
										n2\HP = Max(n2\HP - fps\Factor[0] * 0.5, 0.0)
										If n2\HP =< 0.0 Then NPCIsDead(n2, NPC_IS_DEAD)
									EndIf
								Case NPCType049
									;[Block]
...
Function UpdateNPCType939%(n.NPCs)
	...
	Select n\State
		Case 1.0 ; ~ Becomes angry and relaxed
			;[Block]
			If n\Frame < 1492.0
				AnimateNPC(n, Min(AnimTime(n\OBJ), 1443.0), 1492.0, 0.32, False)
				If (PrevFrame < 1455.0 And n\Frame >= 1455.0) Then PlaySoundEx(snd_I\Step2SFX[Rand(3, 6)], Camera, n\Collider)
				If PrevFrame < 1460.0 And n\Frame >= 1460.0
			...
			Else
				If n\Frame < 2072.0
					AnimateNPC(n, 1493.0, 1580.0, 0.25)
					n\State3 = Max(n\State3 - fps\Factor[0], 0.0)
					If n\Frame > 1579.9 And n\State3 <= 0.0 Then SetNPCFrame(n, 2072.0)
				Else
					AnimateNPC(n, 2072.0, 2090.0, 0.32, False)
					If n\Frame > 2089.9
						SetNPCFrame(n, 770.0)
						n\State = 2.0
					EndIf
			...
			EndIf
			;[End Block]
		Case 2.0 ; ~ Walking and idling between waypoints
			;[Block]
				...
				Else
					...
					Select n\TempState
						Case 1.0
							...
							If (PrevFrame < 98.0 And n\Frame >= 98.0) Lor (PrevFrame < 135.0 And n\Frame >= 135.0) Lor (PrevFrame < 180.0 And n\Frame >= 180.0) Lor (PrevFrame < 187.0 And n\Frame >= 187.0) Then PlaySoundEx(snd_I\Step2SFX[Rand(3, 6)], Camera, n\Collider)
							If n\Frame > 189.9 Then Temp = True
							;[End Block]
						Case 2.0
							;[Block]
							AnimateNPC(n, 191.0, 396.0, 0.25, False)
							If (PrevFrame < 208.0 And n\Frame >= 208.0) Lor (PrevFrame < 280.0 And n\Frame >= 280.0) Lor (PrevFrame < 298.0 And n\Frame >= 298.0) Lor (PrevFrame < 340.0 And n\Frame >= 340.0) Then PlaySoundEx(snd_I\Step2SFX[Rand(3, 6)], Camera, n\Collider)
							If n\Frame > 395.9 Then Temp = True
							;[End Block]
						Case 3.0
							;[Block]
							AnimateNPC(n, 397.0, 556.0, 0.25, False)
							If (PrevFrame < 470.0 And n\Frame >= 470.0) Lor (PrevFrame < 550.0 And n\Frame >= 550.0) Then PlaySoundEx(snd_I\Step2SFX[Rand(3, 6)], Camera, n\Collider)
							If n\Frame > 555.9 Then Temp = True
							;[End Block]
					End Select
					; ~ Change waypoint
					If Temp
	...
	If EntityDistanceSquared(n\Collider, me\Collider) < 49.0
		If EntityVisible(me\Collider, n\Collider) Then GiveAchievement("939")
	EndIf
...
Function UpdateNPCType1048_A%(n.NPCs)
	...
	If n\IsDead > NPC_IS_NOT_DEAD Then Return
...
Function UpdateNPCTypeApache%(n.NPCs)
	...
	If EntityDistanceSquared(me\Collider, n\Collider) < 3600.0 And n\State < 4.0
		...
		Else
			Dist2 = 1.0
		EndIf
		n\SoundCHN = LoopSoundEx(NPCSound[SOUND_NPC_APACHE_PROPELLER], n\SoundCHN, Camera, n\Collider, 25.0, Dist2)
	EndIf
	...
	Select n\State
		Case 1.0 ; ~ Aims and shoots at the player
			...
			If (Not me\Terminated)
				...
				If Dist < DetectDistance
					...
					PositionEntity(Pvt, EntityX(n\Collider), EntityY(n\Collider) - n\CollRadiusH + n\CollRadiusCenter, EntityZ(n\Collider))
		...
		Case 2.0 ; ~ Shoots
			...
			If n\Reload = 0.0
				...
				Pvt = GetDummyPivot(EntityX(n\Collider), EntityY(n\Collider) - n\CollRadiusH + n\CollRadiusCenter, EntityZ(n\Collider))
				RotateEntity(Pvt, EntityPitch(n\Collider), EntityYaw(n\Collider), 0.0, True)
				MoveEntity(Pvt, 0.0622, 0.83925, 0.5351)
...
Function UpdateNPCTypeMTF%(n.NPCs)
	...
	Else
		...
		Local MyBossIsNotDead% = (MyBoss <> Null And MyBoss\IsDead = NPC_IS_NOT_DEAD)
		...
		Select n\State
			Case MTF_WANDERING_AROUND
				...
				; ~ Set a timer to step back if someone is close to the leader
				;[Block]
				If MyBossIsNotDead
...
;							TODO: Contain 106
				...
				Else
					If n\PathStatus = PATH_STATUS_NOT_FOUND
						; ~ I don't have a path, I'm standing still
						;[Block]
						n\CurrSpeed = 0.0
						If Rand(35) = 1 Then RotateEntity(n\Collider, 0.0, Rnd(360.0), 0.0, True)
						FinishWalking(n, 488.0, 522.0, n\Speed * 26.0)
						...
						;[End Block]
					ElseIf n\PathStatus = PATH_STATUS_FOUND
						; ~ I have a path, I'm following it
						;[Block]
						If n\Path[n\PathLocation] = Null
							If n\PathLocation > MaxPathLocations - 1
								n\PathLocation = 0 : n\PathStatus = PATH_STATUS_NO_SEARCH
					...
					Else
						If MyBoss = Null
							; ~ I'm a leader, I'm standing still
							;[Block]
							n\CurrSpeed = 0.0
							If Rand(35) = 1 Then RotateEntity(n\Collider, 0.0, Rnd(360.0), 0.0, True)
							FinishWalking(n, 488.0, 522.0, n\Speed * 26.0)
						...
						Else
							...
							If n\State3 =< 0.0
								...
								;[End Block]
							Else
								; ~ I'm stepping back from the leader
								;[Block]
								n\CurrSpeed = CurveValue(-n\Speed, n\CurrSpeed, 20.0)
				...
				If n\TargetUpdateTimer =< 0.0
					If n\Target = Null And (Not me\Terminated)
						PlayerSeeAble = NPCSeesPlayer(n, 5.0 - me\CrouchState)
						If PlayerSeeAble > 0
							If n\LastSeen > 0.0 And n\LastSeen < 70.0 * 10.0
								If PlayerSeeAble < 2
								...
								If n2\NPCType = NPCTypeMTF
									If n2\State < MTF_173_SPOTTED
										If EntityDistanceSquared(n\Collider, n2\Collider) < 36.0
											...
											n2\State2 = 70.0 * (15.0 * PlayerSeeAble) ; ~ Give up after 10 seconds (20 seconds if detected by loud noise, over camera: 30)
					...
					For n2.NPCs = Each NPCs
						If n2\IsDead = NPC_IS_NOT_DEAD And n\State <> 66.0
							Select n2\NPCType
								Case NPCType049_2
									;[Block]
									If NPCSeesNPC(n2, n) = 1
									...
									EndIf
									;[End Block]
								Case NPCType008_1_Surgeon, NPCType008_1
									;[Block]
									If NPCSeesNPC(n2, n) = 1
										n\EnemyX = EntityX(n2\Collider, True)
										n\EnemyY = EntityY(n2\Collider, True)
			...
			Case MTF_SEARCHING_PLAYER
				...
				;[Block]
				If n\State2 =< 0.0 Lor me\Terminated
					If MyBoss = Null
						If me\Terminated
						...
						If MTFCameraCheckTimer = 0.0
							If Rand(12 - (6 * SelectedDifficulty\AggressiveNPCs)) = 1 ; ~ Maybe change this to another chance -- ENDSHN
								PlayAnnouncement("SFX\Character\MTF\AnnouncCameraCheck.ogg")
								MTFCameraCheckTimer = 2.0
							EndIf
						EndIf
					EndIf
					n\EnemyX = 0.0 : n\EnemyY = 0.0 : n\EnemyZ = 0.0
				...
				If PlayerSeeAble = 1
					; ~ I see the player, I must kill him!
					;[Block]
					...
					; ~ I'm close enough, start shooting at the player
					n\EnemyX = EntityX(me\Collider, True)
					n\EnemyY = EntityY(me\Collider, True)
					n\EnemyZ = EntityZ(me\Collider, True)
				...
				; ~ Primary task is reacting on SCPs
				;[Block]
				; ~ B3D doesn't do short-circuit evaluation, so this retarded nesting is an optimization
				If n\TargetUpdateTimer =< 0.0
					If n_I\Curr173\Idle < 2
			...
			Case MTF_173_SPOTTED
				;[Block]
				; ~ Target is lost/contained, return back to normal state
				;[Block]
				...
				n\State2 = Max(n\State2 - fps\Factor[0], 0.0)
				If NPCSeesNPC(n\Target, n) = 1
					; ~ I see SCP-173, try to contain it!
					;[Block]
					...
					If Curr173Dist < 25.0
						...
						If Curr173Dist < TempDist
							...
							;[End Block]
						Else
							; ~ Come close to SCP-173 and contain!
							;[Block]
							n\CurrSpeed = CurveValue(n\Speed, n\CurrSpeed, 20.0)
							TranslateEntity(n\Collider, Cos(EntityYaw(n\Collider, True) + 90.0) * n\CurrSpeed * fps\Factor[0], 0.0, Sin(EntityYaw(n\Collider, True) + 90.0) * n\CurrSpeed * fps\Factor[0], True)
							AnimateNPC(n, 488.0, 522.0, n\CurrSpeed * 26.0)
				...
				Else
					...
					Else
						If n\PathStatus = PATH_STATUS_FOUND
							...
							;[End Block]
						Else
							; ~ I'm trying to find SCP-173 that hides from me
							;[Block]
							PositionEntity(n\OBJ, n\EnemyX, n\EnemyY, n\EnemyZ, True)
			...
			Case MTF_049_066_106_SPOTTED
				...
				; ~ Target is lost, go back to the normal state
				;[Block]
				If n\State2 <= 0.0
					n\Target = Null
					n\EnemyX = 0.0 : n\EnemyY = 0.0 : n\EnemyZ = 0.0
				...
				; ~ I'm far enough, stop panic!
				If n\State2 > 70.0 And Dist > 225.0 Then n\State2 = 70.0
				
				; ~ Set a timer to step back if someone is close to SCP or leader
				;[Block]
				If Dist < 4.0 And n\Target <> n_I\Curr066 Then n\State3 = 70.0 * 3.0
				If n\State3 > 0.0
					n\Speed = 0.02
					ErasePath(n)
					n\PathTimer = 1.0
				...
				Else
					...
					Else
						If n\State3 =< 0.0
							...
							;[End Block]
						Else
							; ~ I'm stepping back from the target
							;[Block]
							PointEntity(n\Collider, n\Target\Collider)
							RotateEntity(n\Collider, 0.0, EntityYaw(n\Collider, True), 0.0, True)
							n\CurrSpeed = CurveValue(-n\Speed, n\CurrSpeed, 20.0)
			...
			Case MTF_ZOMBIES_SPOTTED
				...
				; ~ Target is lost/terminated, go back to the normal state
				;[Block]
				If n\Target = Null Lor n\State2 <= 0.0 Lor n\Target\IsDead > NPC_IS_NOT_DEAD
					If n\Target\IsDead > NPC_IS_NOT_DEAD
				...
				n\State2 = Max(n\State2 - fps\Factor[0], 0.0)
				If NPCSeesNPC(n\Target, n) = 1
					; ~ I see the target, I must kill it!
					;[Block]
					n\State2 = 70.0 * 10.0
					
					; ~ Set a timer to step back if someone is close to me
					;[Block]
					Dist = EntityDistanceSquared(n\Collider, n\Target\Collider)
					If Dist < 4.0 Then n\State3 = 70.0
					If n\State3 > 0.0
						ErasePath(n)
						n\PathTimer = 1.0
						n\State3 = Max(n\State3 - fps\Factor[0], 0.0)
					...
					; ~ I'm close enough, start shooting at the target
					n\EnemyX = EntityX(n\Target\Collider, True)
					n\EnemyY = EntityY(n\Target\Collider, True)
					n\EnemyZ = EntityZ(n\Target\Collider, True)
