...
Function UpdateNPCType049%(n.NPCs)
	...
	Else
		...
		If Dist < 0.265 And (Not chs\NoTarget)
			If n\State <> 3
				If EntityVisible(me\Collider, n\Collider)
					If wi\HazmatSuit > 0
						me\RemoveHazmatTimer = me\RemoveHazmatTimer - (fps\Factor[0] * 2.5)
						If me\RemoveHazmatTimer < 350.0 And me\RemoveHazmatTimer + fps\Factor[0] * 2.5 >= 350.0 And (Not ChannelPlaying(n\SoundCHN2))
						...
						ElseIf me\RemoveHazmatTimer =< 0.0
							For i = 0 To 2
								If me\RemoveHazmatTimer < -(i * (250.0 * (wi\HazmatSuit = 4))) And me\RemoveHazmatTimer + fps\Factor[0] * 2.5 >= -(i * (250.0 * (wi\HazmatSuit = 4)))
									me\CameraShake = 2.0
									If i = 2
					...
					ElseIf I_714\Using > 0
						...
						Else
							me\Remove714Timer = me\Remove714Timer - (fps\Factor[0] * (4.5 - I_714\Using))
		...
		Select n\State
			...
			Case 2.0 ; ~ Being active
				...
				If Dist < PowTwo(HideDistance * 2.0) And n\Idle = 0 And PlayerInReachableRoom(True)
					n\State2 = Max(n\State2 - fps\Factor[0], 0.0)
					PlayerSeeable = NPCSeesPlayer(n, 8.0)
					If n\State2 > 0.0
				...
				ElseIf n\Idle = 0
					...
					If PlayerInReachableRoom(True) And InFacility = NullFloor ; ~ Player is in a room where SCP-049 can teleport to
						If Rand(8 - (6 * SelectedDifficulty\AggressiveNPCs) + (2 * (Not RemoteDoorOn))) < 3
							TeleportCloser(n)
						Else
			...
			Case 6.0 ; ~ Stunned by Fine SCP-513
				...
				If n\Frame < 1224.0
					StopChannel(n\SoundCHN2) : n\SoundCHN2 = 0
					SetNPCFrame(n, 1225.0)
					n\SoundCHN2 = PlaySoundEx(LoadTempSound("SFX\SCP\049\714Equipped.ogg"), Camera, n\Collider, 10.0, 1.0, True)
				Else
					AnimateNPC(n, 1225.0, 1328.0, 0.36, False)
					
					If (PrevFrame < 1245.0 And n\Frame >= 1245.0) Lor (PrevFrame < 1270.0 And n\Frame >= 1270.0) Then PlaySoundEx(snd_I\Step2SFX[Rand(7, 9)], Camera, n\Collider, 8.0, Rnd(0.8, 1.0))
					If n\Frame > 1327.9 Then n\State = 2.0
				EndIf
				;[End Block]
		End Select
		...
		If Dist < 4.0 And (Not chs\NoTarget)
			If n\State < 2.0 And EntityVisible(me\Collider, n\Collider)
				...
				ElseIf Dist < 0.64
					...
					me\CurrCameraZoom = 20.0
					me\BlurTimer = 500.0
					me\Injuries = me\Injuries + (fps\Factor[0] * 0.0034)
		...
		Select n\State
			Case 0.0 ; ~ Idles
				...
				Else
					...
					If NPCSeesPlayer(n, 4.0) = 1 Then n\State = Rand(4)
				EndIf
				;[End Block]
			Case 1.0, 2.0 ; ~ Echo
				;[Block]
			...
			Case 5.0, 6.0, 8.0 ; ~ Walking or chasing
				;[Block]
				If n\State = 8.0
					If NPCSeesPlayer(n, 6.0) = 1 ; ~ Chasing the player
						PointEntity(n\Collider, me\Collider)
						RotateEntity(n\Collider, 0.0, EntityYaw(n\Collider, True), 0.0, True)
						n\CurrSpeed = CurveValue(n\Speed, n\CurrSpeed, 10.0)
						MoveEntity(n\Collider, 0.0, 0.0, n\CurrSpeed * fps\Factor[0])
...
Function UpdateNPCType1048_A%(n.NPCs)
	...
	Select n\State
		...
		Case 1.0 ; ~ Walks
			...
			Local PlayerSeeable% = NPCSeesPlayer(n, 8.0, 360.0)
	...
	Select n\State
		...
		Case 2.0 ; ~ Shoots
			...
			If n\Reload = 0.0
				...
				Pvt = GetDummyPivot(EntityPitch(n\Collider), EntityYaw(n\Collider), 0.0, True)
				PositionEntity(Pvt, EntityX(n\OBJ), EntityY(n\OBJ), EntityZ(n\OBJ))
				MoveEntity(Pvt, 0.0622, 0.83925, 0.5351)
...
;							TODO: Contain 106
				...
				If n\TargetUpdateTimer =< 0.0
					If n\Target = Null
						PlayerSeeAble = NPCSeesPlayer(n, 4.0)
						If PlayerSeeAble > 0
