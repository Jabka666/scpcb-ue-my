...
Function AddLight.Lights(room.Rooms, x#, y#, z#, LType%, Range#, R%, G%, B%, HasSprite% = True, SpriteScale# = 1.0, CastShadows% = True, FOV# = 90.0)
	...
	If room <> Null
		If Rand(50) = 1 And (Not (CastShadows Lor HasSprite)) ; ~ TODO: Use scripted instead?
...
Function PlaceForest%(fr.Forest, x#, y#, z#, r.Rooms)
	...
	hMap[ROOM1] = LoadTexture_Strict("GFX\Map\Forest\forest1h.png", 1 + 131072)
	Mask[ROOM1] = LoadTexture_Strict("GFX\Map\Forest\forest1h_mask.png", 1 + 2 + 131072)
	
	hMap[ROOM2] = LoadTexture_Strict("GFX\Map\Forest\forest2h.png", 1 + 131072)
	Mask[ROOM2] = LoadTexture_Strict("GFX\Map\Forest\forest2h_mask.png", 1 + 2 + 131072)
	
	hMap[ROOM2C] = LoadTexture_Strict("GFX\Map\Forest\forest2Ch.png", 1 + 131072)
	Mask[ROOM2C] = LoadTexture_Strict("GFX\Map\Forest\forest2Ch_mask.png", 1 + 2 + 131072)
	
	hMap[ROOM3] = LoadTexture_Strict("GFX\Map\Forest\forest3h.png", 1 + 131072)
	Mask[ROOM3] = LoadTexture_Strict("GFX\Map\Forest\forest3h_mask.png", 1 + 2 + 131072)
	
	hMap[ROOM4] = LoadTexture_Strict("GFX\Map\Forest\forest4h.png", 1 + 131072)
	Mask[ROOM4] = LoadTexture_Strict("GFX\Map\Forest\forest4h_mask.png", 1 + 2 + 131072)
...
Const r_room2c_lcz% = 28, r_room2c_2_lcz% = 29, r_room2c_3_lcz% = 30
Const r_room2c_gw_lcz% = 31
...
Const r_room4_lcz% = 39, r_room4_2_lcz% = 40, r_room4_3_lcz% = 41
Const r_room4_ic% = 42
; ~ CHECKPOINT
...
Const r_room2_mt% = 56
Const r_room2_nuke% = 57
Const r_room2_servers_hcz% = 58
...
Const r_room2c_hcz% = 66, r_room2c_2_hcz% = 67, r_room2c_3_hcz% = 68
Const r_cont2c_096% = 69
Const r_room3_hcz% = 70, r_room3_2_hcz% = 71, r_room3_3_hcz% = 72
Const r_cont3_009% = 73, r_cont3_966% = 74
Const r_room4_hcz% = 75, r_room4_2_hcz% = 76, r_room4_3_hcz% = 77
Const r_room4_gw% = 78
; ~ CHECKPOINT
Const r_room2_checkpoint_hcz_ez% = 79
; ~ EZ
...
Const cs_ue% = 10
Const cs_logo% = 11
Const cs_reminder% = 12
Const cs_not_clickable% = 13
;[End Block]
...
Function FindChatScreenEventID%(ChatScreenName$)
	...
	Select ChatScreenName
		...
		Case "screen_860.png"
			...
			Return(cs_860)
			;[End Block]
		Case "screen_1025.png"
			;[Block]
			Return(cs_1025)
			;[End Block]
		Case "screen_1499.png"
			;[Block]
			Return(cs_1499)
			;[End Block]
...
Function SetScreenTexture%(s.Screens)
	Select s\ScreenEventID
		Case cs_attention, cs_error, cs_reminder, cs_427, cs_500, cs_714, cs_860, cs_1025, cs_1499
			;[Block]
					...
					Select s\ScreenEventID
						Case cs_clickable
							...
							Else
								...
								If Rand(3000 - (1200 * (Not RemoteDoorOn)) - (1200 * SelectedDifficulty\AggressiveNPCs)) = 1 And s\State2 = 0.0
									If s\Display096
										If EntityInView(s\OBJ, Camera) And EntityVisible(s\OBJ, Camera)
							...
							EndIf
							;[End Block]
						Case cs_attention, cs_error, cs_427, cs_500, cs_714, cs_860, cs_1025, cs_1499
							;[Block]
					...
					If Int(r\x / RoomSpacing) = x And Int(r\z / RoomSpacing) = y
						...
						If ShouldSpawnDoor
							If x + 1 < MapGridSize + 1
								If CurrMapGrid\Grid[(x + 1) + (y * MapGridSize)] > MapGrid_NoTile Then r\AdjDoor[0] = CreateDoor(r, FloatX + (RoomSpacing / 2.0), 0.0, FloatY, 90.0, Max(Rand(-3, 1), 0), DoorType)
							EndIf
						EndIf
