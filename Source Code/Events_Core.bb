...
;				;[End Block]
			...
			Case e_cont1_914
				...
				If PlayerRoom = e\room
					...
					If e\EventState > 0.0
						...
						CanSave = 0
						If (Not n_I\Curr106\Contained)
							If n_I\Curr106\State > 0.0 Then n_I\Curr106\State = n_I\Curr106\State - (fps\Factor[0] * (1.5 + (SelectedDifficulty\AggressiveNPCs)))
						EndIf
			...
			Case e_room2_nuke
				...
				If PlayerRoom = e\room
					...
					If y > 6.0 Then InFacility = LowerFloor
					e\EventState2 = UpdateElevators(e\EventState2, e\room\RoomDoors[0], e\room\RoomDoors[1], e\room\Objects[0], e\room\Objects[1], e)
							...
							If it\Dropped = 1 Lor it\Dropped = -1
								...
								If it\Dropped = 1 And Abs(x) < 264.0
									For i = -1 To 1 Step 2
										...
										it2.Items = CreateItem(it\Name, it\ItemTemplate\TempName, TFormedX(), EntityY(it\Collider), TFormedZ(), it\R, it\G, it\B, it\Alpha)
...
Const INTRO_CELL_OPENED% = 2
Const INTRO_MOVING_TO_CHAMBER% = 3
Const INTRO_ESCORT_DONE% = 4
Const INTRO_IN_CHAMBER% = 5
;[End Block]
		...
		If e\EventID = e_cont1_173_intro
			...
			If PlayerRoom = e\room
				...
				If e\EventState = 0.0
					...
					e\room\NPC[5] = CreateNPC(NPCTypeGuard, EntityX(e\room\Objects[9], True), EntityY(e\room\Objects[9], True), EntityZ(e\room\Objects[9], True))
					e\room\NPC[5]\State = 7.0
					e\room\NPC[5]\Sound2 = LoadSound_Strict("SFX\Room\Intro\Guard\Music" + Rand(5) + ".ogg")
					RotateEntity(e\room\NPC[5]\Collider, 0.0, e\room\Angle + 180.0, 0.0, True)
					...
					Select e\EventState
						Case INTRO_IN_CELL
							...
							If e\EventState2 < 15.0
								...
								If e\EventState2 < 14.0
									...
									Else
										y = (y + 0.2) + (0.302 + 0.6 - (y + 0.2)) * Max((e\EventState2 - 10.0) / 4.0, 0.0) 
									EndIf
						...
						Case INTRO_CELL_OPENED
							...
							If DistanceSquared(EntityX(me\Collider), e\room\x - 4096.0 * RoomScale, EntityZ(me\Collider), e\room\z + 192.0 * RoomScale) > 2.25
								...
								LoadNPCSound(e\room\NPC[3], "SFX\Room\Intro\Guard\Ulgrin\Escort" + Rand(2) + ".ogg", 1)
								e\room\NPC[3]\SoundCHN2 = PlaySound2(e\room\NPC[3]\Sound2, Camera, e\room\NPC[3]\Collider, 10.0, 1.0, True)
						...
						Case INTRO_MOVING_TO_CHAMBER
							...
							If (Not ChannelPlaying(e\room\NPC[3]\SoundCHN2))
								If e\room\NPC[3]\State <> 11.0
									If Dist < 9.0
										...
										e\room\NPC[3]\State = 3.0
									Else
										e\room\NPC[3]\CurrSpeed = CurveValue(0.0, e\room\NPC[3]\CurrSpeed, 10.0)
										If e\room\NPC[3]\State <> 9.0 And e\room\NPC[3]\CurrSpeed =< 0.001
											; ~ Play this sound only once
											If e\room\NPC[3]\State3 = 0.0
												StopChannel(e\room\NPC[3]\SoundCHN) : e\room\NPC[3]\SoundCHN = 0
							...
							e\room\NPC[5]\SoundCHN2 = LoopSound2(e\room\NPC[5]\Sound2, e\room\NPC[5]\SoundCHN2, Camera, e\room\NPC[5]\OBJ, 2.0, 0.5)
							...
							If e\room\NPC[3]\State <> 11.0
								If DistanceSquared(EntityX(e\room\NPC[3]\Collider), EntityX(e\room\RoomDoors[2]\FrameOBJ, True), EntityZ(e\room\NPC[3]\Collider), EntityZ(e\room\RoomDoors[2]\FrameOBJ, True)) < 20.25 And Dist < 25.0
									...
									For i = 3 To 4
										...
										e\room\NPC[i]\State = 9.0
									Next
									LoadNPCSound(e\room\NPC[3], "SFX\Room\Intro\Guard\Ulgrin\EscortDone" + Rand(5) + ".ogg")
									e\room\NPC[3]\SoundCHN = PlaySound2(e\room\NPC[3]\Sound, Camera, e\room\NPC[3]\Collider, 10.0, 1.0, True)
									...
									If DistanceSquared(EntityX(me\Collider), EntityX(e\room\NPC[5]\Collider), EntityZ(me\Collider), EntityZ(e\room\NPC[5]\Collider)) < 25.0 And Dist > 25.0
										If EntityVisible(me\Collider, e\room\NPC[5]\Collider)
											For i = 3 To 5
												...
												e\room\NPC[i]\State = 11.0 : e\room\NPC[i]\State3 = 1.0 : e\room\NPC[i]\Reload = 70.0 * 3.0
											Next
											e\room\NPC[5]\SoundCHN = PlaySound2(LoadTempSound("SFX\Room\Intro\Guard\PlayerEscape.ogg"), Camera, e\room\NPC[5]\Collider, 10.0, 1.0, True)
										EndIf
									EndIf
								EndIf
							EndIf
							
							If e\room\NPC[3]\State = 9.0
